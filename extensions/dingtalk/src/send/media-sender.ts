...
export async function uploadMediaItem(
    ...
    const { type, path: filePath, name } = item;
    ...
    try {
        ...
        logger?.error?.({ path: filePath, err: { message: (err as Error)?.message } }, "Failed to stat media file");
        return { ok: false, error };
    }
    ...
    try {
        ...
        logger?.error?.({ path: filePath, err: { message: (err as Error)?.message } }, "Media processing error");
        return { ok: false, error };
    }
}
...
export async function sendMediaItem(
    ...
    return await sendMediaWithId(type, uploadResult.mediaId, uploadResult.fileName!, sessionWebhook, replyLogger);
}
...
async function sendMediaWithId(
    ...
    switch (type) {
        ...
        default:
            logger?.warn?.({ type }, "Unknown media type");
            return { ok: false, error: `未知媒体类型: ${type}` };
    }
}
...
export async function processMediaItems(
    ...
    for (const item of items) {
        ...
        if (result.ok) {
            ...
            logger?.warn?.({ path: item.path, type: item.type, error: result.error }, "Media send failed");
        }
    }
